...
{
    public function createRecurringBookings($requestData)
    {
        ...
        while ($requestData['end_time']->lte($recurringUntil)) {
            $this->createBooking($requestData);
            $requestData['start_time']->addWeek();
            $requestData['end_time']->addWeek();
        }
    }

    public function createBooking($requestData)
    {
    ...
    public function isRoomTaken($requestData)
    {
        ...
        $bookings       = Booking::where('room_id', $requestData['room_id'])->get();

        do {
            if (
                $bookings->where('start_time', '<', $start_time)->where('end_time', '>', $start_time)->count() ||
                $bookings->where('start_time', '<', $end_time)->where('end_time', '>', $end_time)->count() ||
                $bookings->where('start_time', '<', $start_time)->where('end_time', '>', $end_time)->count()
            ) {
                return true;
            }
